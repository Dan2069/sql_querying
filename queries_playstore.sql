...
SELECT id FROM analytics WHERE id IN (1880);
...
SELECT reviews FROM analytics WHERE rating >= 4.8 ORDER BY reviews DESC;
...
--8 Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
 SELECT rating FROM analytics WHERE min_installs < 50 ORDER BY ra
ting DESC;
--9 Find the names of all apps that are rated less than 3 with at least 10000 reviews.
 SELECT app_name FROM analytics WHERE rating < 3 AND reviews > 10
000;
--10 Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
SELECT reviews FROM analytics WHERE price BETWEEN 0.10 AND 1.00 ORDER BY reviews DESC LIMIT 10;
...
SELECT app_name, last_updated FROM analytics ORDER BY last_updated ASC;
...
proportion FROM analytics WHERE min_installs >= 100000 ORDER BY proportion D
ESC LIMIT 1;